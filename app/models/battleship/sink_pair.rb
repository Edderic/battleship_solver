...
module Battleship
  class SinkPair
    ...
    def ship
      @table.unsunk_ships.select {|ship| ship.length == @sink_ship_length}.first
    end
    ...
    def sinkable?
      ship.occupies_point?(@sink_point) && ship.occupied_points.all? do |point|
        if point.same_as?(@sink_point)
          true
        else
          table.point_at(point).hit?
        end
      end
    end

    def occupied_points
      Battleship::PointsDecorator.new( sinkable? ? ship.occupied_points : [])
    end

    def sink!
      tables_generator.hits.each do |hit|
        tables_generator.sunken_points << hit
        tables_generator.hits.reject! {|some_hit| some_hit.same_as?(hit) }
      end
